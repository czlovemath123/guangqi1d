...
subroutine read_tree(file_id)
    ...
    if (lradhydro_boost) call initialize_iso_temp()
...
subroutine rebuild_tree_list(keys)
    ...
    blk_root%curve='Z'
    if (lreassign_tree_curve) then      !some old h5 files uses Z order, need to reassign
        blk_root%curve='Z'
    end if
    ...
    call reassign_curve_type(blk_root)
