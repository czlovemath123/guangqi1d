...
subroutine dsetname_filename()
    ...
    if (iradiation==1) then
        ...
        if (lirradiation) then
            dsetname(output_var_number+1)='irrad_tau'
            dsetname(output_var_number+2)='aradx_irrad'
...
subroutine write_blk_dset_xsurface_parallel(blk, group_id, dims, dsname, plist_id)
    ...
    ! Use a static array with save attribute to avoid repeated allocation/deallocation
    real(4), dimension(:,:), allocatable, save :: xsurf_data
    logical, save :: first_call = .true.
    integer, save :: saved_nx = -1, saved_ny = -1
    
    ! Only allocate if needed or if dimensions changed
    if (first_call .or. saved_nx /= blk_size_nx .or. saved_ny /= blk_size_ny) then
        if (allocated(xsurf_data)) deallocate(xsurf_data)
        allocate(xsurf_data(0:blk_size_nx, blk_size_ny))
        saved_nx = blk_size_nx
        saved_ny = blk_size_ny
        first_call = .false.
    end if
    ...
    case ('irrad_tau')
        xsurf_data = blk%tau(0:blk_size_nx, 1:blk_size_ny, 1)
    case ('torque_work')
        ! This case doesn't set xsurf_data, so we need to handle it
        xsurf = .false.
    ...
    ! Note: We don't deallocate xsurf_data here to reuse it in subsequent calls
...
subroutine write_blk_dset_cell_parallel(blk, group_id, dims, dsname, plist_id)
    ...
    ! Use a static array to avoid repeated allocation/deallocation
    real(4), dimension(:,:), allocatable, save :: cell_data
    logical, save :: first_call = .true.
    integer, save :: saved_nx = -1, saved_ny = -1
    
    ! Only allocate if needed or if dimensions changed
    if (first_call .or. saved_nx /= blk_size_nx .or. saved_ny /= blk_size_ny) then
        if (allocated(cell_data)) deallocate(cell_data)
    ...
    case ('aradx_irrad')
        cell_data = blk%aradx_irrad(1:blk_size_nx, 1:blk_size_ny, 1)
    case ('H2')
    ...
    ! Note: We don't deallocate cell_data here to reuse it in subsequent calls
...
recursive subroutine save_key(blk,keys,i)
    ...
    type(blockdef), pointer :: blk
    integer, dimension(:,:), allocatable :: keys
    integer :: i,j,key_p(3),key(3),idx,idy
