...
subroutine petsc_Ab_1d_interior(blk,i,dt,A,b)
    ...
    type(blockdef), pointer :: blk
    real(8) :: dx,dt,beta,alpha_xl,alpha_xu,gxl,gxu,D_xl,D_xu,cv,er,temp,v,xl,xc,xr,sxl,sxr,vol,rho,pradgradv
    ...
    pradgradv=blk%fld_pradgradv(i,1,1)*dt
    cv=blk%cv(i,1,1)
    er=blk%Erad(i,1,1)
    temp=blk%temp(i,1,1)
    ...
    jj=ii_E
    v=1+(alpha_xl*gxl*D_xl+alpha_xu*gxu*D_xu+beta+pradgradv)*const_Erad
    call MatSetValue(A,ii_E,jj,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    jj=ii_E+2
    ...
    if (lrad_adv) then
        v=er-(3*beta*a_rad*temp**4)*const_Erad+blk%erad_adv(i,1,1)*dt
    else
    ...
    if (rho<rho_thresh_petsc1) then
        ...
        v=cv*temp+3*beta*a_rad*temp**4+blk%erad_source(i,1,1)*dt
    end if
...
subroutine petsc_Ab_1d_lb(dt,A,b)
    type(blockdef), pointer :: blk
    real(8) :: dt,beta,alpha_xl,alpha_xu,gxl,gxu,D_xl,D_xu,cv,er,temp,v,dx,xl,xc,xr,sxl,sxr,vol,rho,pradgradv
    ...
    i=1
    call assign_beta_temp_D_1d(blk,i,dt,beta,temp,D_xl,D_xu)
    pradgradv=blk%fld_pradgradv(1,1,1)*dt
    cv=blk%cv(1,1,1)
    er=blk%Erad(1,1,1)
    temp=blk%temp(1,1,1)
    dx=blk%dxyz(1)
    ...
    if (rad_bound_type(1)==2.or.rad_bound_type(1)==8) then
        v=1+(alpha_xu*gxu*D_xu+beta+pradgradv)*const_Erad
    ...
    if (rad_bound_type(1)==2.or.rad_bound_type(1)==8) then
        if (lrad_adv) then
            v=er-(3*beta*a_rad*temp**4)*const_Erad+blk%erad_adv(1,1,1)*dt
        else
            v=er-(3*beta*a_rad*temp**4)*const_Erad
        end if
    else if (rad_bound_type(1)==9) then
        if (lrad_adv) then
            v=er-(3*beta*a_rad*temp**4-alpha_xl*gxl*D_xl*blk%Erad(0,1,1))*const_Erad+blk%erad_adv(1,1,1)*dt
        else
            v=er-(3*beta*a_rad*temp**4-alpha_xl*gxl*D_xl*blk%Erad(0,1,1))*const_Erad
        end if
    end if
    call VecSetValue(b,ii_E,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    if (rho<rho_thresh_petsc1) then
        ...
        v=cv*temp+3*beta*a_rad*temp**4+blk%erad_source(1,1,1)*dt
    end if
    call VecSetValue(b,ii_T,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    nullify(blk)
...
subroutine petsc_Ab_1d_rb(dt,A,b)
    type(blockdef), pointer :: blk
    real(8) :: dt,beta,alpha_xl,alpha_xu,gxl,gxu,D_xl,D_xu,cv,er,temp,v,dx,q,xl,xc,xr,sxl,sxr,vol,rho,pradgradv
    integer :: i,ii_E,ii_T,jj,ierr
    Mat A
    Vec b
    blk=>blk_tail
    ...
    i=blk_size_nx
    call assign_beta_temp_D_1d(blk,i,dt,beta,temp,D_xl,D_xu)
    pradgradv=blk%fld_pradgradv(blk_size_nx,1,1)*dt
    ...
    if (igeometry==0) then
        if (rad_bound_type(2)==1) then
            ...
            v=1+(beta+alpha_xl*gxl*D_xl+q*alpha_xu*gxu*D_xu/(1d0+q)+pradgradv)*const_Erad
        else if (rad_bound_type(2)==2) then
            v=1+(beta+alpha_xl*gxl*D_xl+pradgradv)*const_Erad
        end if
    else if (igeometry==2) then
        if (rad_bound_type(2)==1) then
            v=1+(alpha_xl*gxl*D_xl+alpha_xu*gxu*D_xu*(xr**2-xc**2)/xr**2+beta+pradgradv)
        else if (rad_bound_type(2)==2) then
            v=1+(alpha_xl*gxl*D_xl+beta+pradgradv)*const_Erad
        end if
    end if
    jj=ii_E
    call MatSetValue(A,ii_E,jj,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    ...
    call MatSetValue(A,ii_T,ii_E,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    if (lrad_adv) then
        v=er-(3*beta*a_rad*temp**4)*const_Erad+blk%erad_adv(blk_size_nx,1,1)*dt
    else
        v=er-(3*beta*a_rad*temp**4)*const_Erad
    end if
    call VecSetValue(b,ii_E,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    if (rho<rho_thresh_petsc1) then
        ...
        v=cv*temp+3*beta*a_rad*temp**4+blk%erad_source(blk_size_nx,1,1)*dt
    end if
    call VecSetValue(b,ii_T,v,INSERT_VALUES,ierr)!;petsccall(ierr)
    nullify(blk)
