...
subroutine communicate_tau_internal_1d(n)
    type(blockdef), pointer :: blk,blk_temp
    type(blockpointer) :: blk_nb
    integer :: n,i,j,neighbour_rank
    do j=1,n
        blk=>llist_head
        do i=1,np_nblk(rank+1)
            if (associated(blk%nb(1)%blk)) then
                ...
                if (neighbour_rank==rank) then
                    blk_temp=>blk_nb%blk
                    blk%tau(0,1,1)=blk_temp%tau(blk_size_nx,1,1)
                end if
            end if
            call irradiation_update_tau(blk)
            blk=>blk%next
        end do
    end do
end subroutine communicate_tau_internal_1d

subroutine communicate_tau_external_1d()
    type(blockdef), pointer :: blk,blk_temp
    type(blockpointer) :: blk_nb
    integer :: n,i,j,neighbour_rank,tag,req,ierr,stat(MPI_STATUS_SIZE)
    real(8) :: tau_send,tau_recv
    blk=>llist_head
    do i=1,np_nblk(rank+1)
        if (associated(blk%nb(2)%blk)) then
            blk_nb=blk%nb(2)
            ...
            if (neighbour_rank/=rank) then
                blk_temp=>blk_nb%blk
                tau_send=blk%tau(blk_size_nx,1,1)
                tag=1;call mpi_isend(tau_send,1,MPI_REAL8,neighbour_rank,tag,MPI_COMM_WORLD,req,ierr)
            end if
        end if
        if (associated(blk%nb(1)%blk)) then
            blk_nb=blk%nb(1)
            ...
            if (neighbour_rank/=rank) then
                blk_temp=>blk_nb%blk
                tag=1;call mpi_recv(tau_recv,1,MPI_REAL8,neighbour_rank,tag,MPI_COMM_WORLD,stat,ierr)
                blk%tau(0,1,1)=tau_recv
            end if
        end if
        blk=>blk%next
    end do
end subroutine communicate_tau_external_1d
...
subroutine prolongation_cell_1d(blk,i,prolonged)
    ...
    if (igeometry==0) then
        ...
        if (llnx) then
            ...
            r2=geometric_si(n_domain(1),xgeo_b,xgeo_dp,blk%level+1,blk_size_nx,iblkx,2*i-1)
